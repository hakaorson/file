...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=10cm]{node-vec.png}
    \caption{结点特征嵌入示意图}
    \label{fig:node-vec}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=10cm]{node-embedding}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=14cm]{gae/autoencoder}
    \caption{自编码器示意图}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=10cm]{gae/mygae}
...
通过蛋白质相互作用数据集构造蛋白质相互作用网络，再在网络的基础上应用多种结点嵌入方法和邻边嵌入方法为网络添加结点特征和邻边特征，其工程如图\ref{fig:featedgraph}所示，其中的小矩形代表某一个元素的特征向量，橘色为结点特征向量，蓝色的邻边特征向量。
